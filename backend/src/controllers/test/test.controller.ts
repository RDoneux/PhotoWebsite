...
export class TestController implements Controller {
  ...
  getAllTestData = async (request: Request, response: Response) => {
    try {
      const tests = (await collections[this.collection]
        .find({})
        .toArray()) as Test[];
  ...
  getTestData = async (request: Request, response: Response) => {
    ...
    try {
      ...
      test
        ...
        : response.status(404).send({ data: `test with id ${id} not found` });
    } catch (error: any) {
      Logger.error(error);
      response.status(500).send({ data: error.message });
    }
  ...
  postTestData = async (request: Request, response: Response) => {
    try {
      const result = await collections[this.collection].insertOne(
        request.body as Test
      );
      result
        ? response.status(201).send({ data: "Successfully inserted new Test" })
  ...
  patchTestData = async (request: Request, response: Response) => {
    ...
    try {
      const result = await collections[this.collection].updateOne(
        { _id: new ObjectId(id) },
        { $set: request.body as Test },
        { upsert: true }
      );
      result
        ? response
            .status(201)
            .send({ data: `Successfully updated test with id: ${id}` })
        : response
            .status(400)
            .send({ data: `Failed to update test with id: ${id}` });
    } catch (error: any) {
      Logger.error(error);
      response.status(500).send({ data: error.message });
    }
  ...
  deleteTestData = async (request: Request, response: Response) => {
    ...
    try {
      ...
      if (result && result.deletedCount) {
        response
          .status(202)
          .send({ data: `Successfully deleted Test with id ${id}` });
      } else if (!result) {
        response
          .status(400)
          .send({ data: `Failed to delete Test with id ${id}` });
